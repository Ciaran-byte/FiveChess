...
/*
名称:ArtificialIntelligence.h
作用:用于通过当前棋局，推断出下棋位置
实例:
	ArtificialIntelligence AI;
	cv::Point p = AI.getPosition(src,WHITE);
*/
//定义全局变量
const int NOTHING = 0;
const int BLACK = 1;
...
public:
	//获取下的最佳位置
	cv::Point getPosition(const cv::Mat&, int color);
private:
	//01 成员变量
	//用来标记基本棋形的
...
//02-3 获取某一个方向的棋子，并且判断是什么基本棋形
	...
	//02-5 综合四个方向进行判分
	int judgeChessSituation(const cv::Mat&, cv::Point p, int color);
