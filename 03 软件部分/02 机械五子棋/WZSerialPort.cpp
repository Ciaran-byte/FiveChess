...
bool WZSerialPort::open  //打开串口
(	const char* portname,  //串口名
	int baudrate, //波特率
	char parity ,  //校验位
	char databit ,  //数据位
	char stopbit,  //停止位
	char synchronizeflag //选择同步还是异步的标记
)
{
	this->synchronizeflag = synchronizeflag; //同步和异步的标记
	HANDLE hCom = NULL; //打开串口的句柄
	...
	if (!SetupComm(hCom, 1024, 1024))// 串口  输入缓冲区  输出缓冲区
	{
	...
	// 配置参数 
	DCB p;
	memset(&p, 0, sizeof(p)); //把参数配置结构DCB全部置零
